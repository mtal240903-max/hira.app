...
export default function IncomingGroupCallBanner() {
  const { groupCallStatus, groupCallInfo, joinGroupCall, declineGroupCall } = useCall();

  if (groupCallStatus !== "group-ringing") return null;

  return (
    <div className="incoming-call">
      <Avatar name={groupCallInfo.fromName} avatarUrl={groupCallInfo.fromAvatar} size={52} />
      <div className="incoming-call__info">
        <p className="incoming-call__name">{groupCallInfo.fromName}</p>
        <p className="incoming-call__type">
          Appel de groupe {groupCallInfo.callType === "video" ? "vidéo" : "audio"} entrant...
        </p>
      </div>
      <div className="incoming-call__actions">
        <button className="incoming-call__btn incoming-call__btn--reject" onClick={declineGroupCall} aria-label="Refuser">
          {ICON_PHONE_OFF}
        </button>
        <button className="incoming-call__btn incoming-call__btn--accept" onClick={joinGroupCall} aria-label="Rejoindre">
          {ICON_PHONE}
        </button>
      </div>
    </div>
